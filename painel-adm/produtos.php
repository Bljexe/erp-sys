      ...
      <div class="card my-4">
        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
          <div class="bg-gradient-secondary shadow-secondary border-radius-lg pt-4 pb-3 d-flex justify-content-between container-fluid">
            <h6 class="text-white text-capitalize ps-3">Cadastro de Produtos</h6>
            ...
            <form method="POST" id="form-produto">
              <div class="modal-body">
                ...
                <div class="row">
                  ...
                  <div class="col-md-6 p-0">
                    <label class="font-weight-bold w-20">Margem de Lucro (%)</label>
                    <?php
                    $lucro = isset($lucro) ? $lucro : '';
                    $lucro_formatado = sprintf("%.1f%%", $lucro);
                    ?>
                    <input type="text" class="px-2 py-1 border-radius-lg w-30 text-sm" style="border: 1px solid #000;" name="lucro" id="lucro" value="<?php echo $lucro_formatado ?>" required>
                  </div>
                  ...
                  <div class="col-md-6 p-0">
                    ...
                    <button type="button" id="calcular-preco" class="btn btn-secondary">Calcular</button>
                  </div>
...
<script>
  $(document).ready(function() {
    $("#calcular-preco").click(function() {
      var custo = parseFloat($("#custo").val().replace(',', '.'));
      var margemLucro = parseFloat($("#lucro").val().replace(',', '.'));

      if (!isNaN(custo) && !isNaN(margemLucro)) {
        var lucro = custo * (margemLucro / 100);
        var precoVenda = custo + lucro;

        $("#venda").val(precoVenda.toFixed(2).replace('.', ','));
        $("#venda-calculado").val(precoVenda.toFixed(2).replace('.', ','));
      } else {
        alert("Por favor, insira valores válidos para Custo e Margem de Lucro.");
      }
    });
  });
</script>