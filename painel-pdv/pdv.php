...
$total_reg = @count($res);
if ($total_reg == 0) {
  echo '<h2 style="text-align: center; margin-top: 50px;">Nenhum caixa aberto, faça a abertura de um CAIXA!</h2>';
    ...
    $.ajax({
      ...
      .done(function(response) {
        ...
        if (response.status === 'success') {
          ...
          var urlComprovante = "comprovante_class.php?id=" + response.id;
      ...
      .fail(function(xhr, status, error) {
        console.log('Retorno do AJAX:', xhr);

        try {
          var responseTextWithoutBOM = xhr.responseText.replace(/^\uFEFF/, '');
          var response = JSON.parse(responseTextWithoutBOM);

          // Verificar se o status no fail é "success" e realizar o redirecionamento
          if (response && response.status === 'success') {
            ...
            console.log('Erro ao salvar venda');
            $.notify("Erro na requisição AJAX", "error");
          }
        } catch (e) {
